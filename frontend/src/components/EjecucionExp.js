...
export default function EjecucionExp() {
  ...
  const [nombreMedicionActual, setNombreMedicionActual] =
    useState("Tabla General");
  ...
  const traerMedicionesRegistrar = async (arregloNFase) => {
    ...
    for (let j = 0; j < medicionesFase.length; j++) {
      const resMediciones = await axios.get(
        routesBD.mediciones + medicionesFase[j]
      );
  ...
  const guardarFaseActiva = async (faseGuardar) => {
    ...
    const resFaseActiva = await axios.put(
      routesBD.experimentos + "faseActiva/" + idUrl["id"],
      faseActivaGuardar
    );
  };
    ...
    if (faseActiva < fasesExp.length) {
      if (fasesExp != "") {
        ...
        if (fasesExp != "") {
          ...
          calcularTiempoFases(
            arregloNFase[faseActiva].tiempoInicio,
            arregloNFase[faseActiva].tiempoFin
          );
        }
  ...
  const onDeleteObs = async (id) => {
    ...
    for (let i = 0; i < arrdelete.length; i++) {
      if (id == arrdelete[i]) {
        ...
        const resq = await axios.put(
          routesBD.fases + "agregarObservaciones/" + fasesExp[faseActiva]._id,
          datoIdObsFase
        );
        setActualizarTabla(!actualizarTabla);
  ...
  const sendData = async () => {
    const dataVis = await axios.get(
      routesBD.experimentosVis +
        idUrl["id"] +
        "/" +
        dataFasesVis[faseActiva][0]._id
    );
    //window.visualizacion.postMessage(dataVis.data, 'http://localhost:3000')
    window.visualizacion.postMessage(dataVis.data, "http://localhost");
  ...
  var json = {
    ...
    GruposFase: [
      {
        idGrupo: "idGrupo00001",
        Participantes: [
          {
            idParticipante: "participante001",
            dispositivos: [
          ...
          {
            idParticipante: "participante003",
            dispositivos: [
          ...
          {
            idParticipante: "participante004",
            dispositivos: [
      ...
      {
        idGrupo: "idGrupo00003",
        Participantes: [
          {
            idParticipante: "participante002",
            dispositivos: [
              { dispositivo: "Cam01", canal: "1" },
              { dispositivo: "Mic01", canal: "1" },
            ],
          ...
          {
            idParticipante: "participante005",
            dispositivos: [
              { dispositivo: "Cam01", canal: "2" },
              { dispositivo: "Mic01", canal: "2" },
            ],
          ...
          {
            idParticipante: "participante006",
            dispositivos: [
              { dispositivo: "Cam02", canal: "3" },
              { dispositivo: "Mic01", canal: "3" },
            ],
  ...
  const closeModalContinuarFase = async (estadoContinuar) => {
    ...
    if (estadoContinuar === "Continuar") {
      ...
      if (faseActiva === fasesExp.length - 1) {
        ...
        const resEtapa = await axios.put(
          routesBD.experimentos + idExperimento,
          Etapa
        );
        setTimeout(function () {
    ...
    for (let i = 0; i < arrTotal.length; i++) {
      ...
      for (let j = 0; j < arrTotal[i]["participantes"].length; j++) {
        const resP = await axios.get(
          routesBD.participantes + arrTotal[i]["participantes"][j]
        );
      ...
      for (let j = 0; j < gruposActuales.length; j++) {
        ...
        for (let k = 0; k < participantes[j].length; k++) {
          for (let h = 0; h < participantes[j][k]["dispositivos"].length; h++) {
            if (
              participantes[j][k]["dispositivos"][h]["tipoDispositivo"] ===
              medicionesActuales[i]["dispositivosAsociados"][0]
            ) {
              let participante = {
                nombre: participantes[j][k]["descripcion"],
                dispositivo:
                  participantes[j][k]["dispositivos"][h]["dispositivo"],
  ...
  const conectarSocket = (arregloMedicionesGruposParticipantes) => {
    ...
    window[socket + "ExperimentControl"] = io.connect(
      "http://192.168.1.128:84/Experiment"
    );
    ...
    for (let i = 0; i < medicionesFaseActual.length; i++) {
      ...
      window[socket + medicionFinal].on("report_metric", function (msg) {
        ...
        if (
          nombreMedicionVariable === "Tabla General" ||
          nombreMedicionVariable === medicionesFaseActual[i]["nombre"]
        ) {
          //console.log(nombreMedicionVariable);
          for (let k = 0; k < medicionesFaseActual[i]["grupos"].length; k++) {
            if (
              medicionesFaseActual[i]["grupos"][k]["participantes"].length > 0
            ) {
              for (
                let l = 0;
                l <
                medicionesFaseActual[i]["grupos"][k]["participantes"].length;
                l++
              ) {
                for (let j = 0; j < msg.data.devices.length; j++) {
                  let dispositivoRecibido = msg.data.devices[j];
                  if (
                    dispositivoRecibido["name"] ===
                    medicionesFaseActual[i]["grupos"][k]["participantes"][l][
                      "dispositivo"
                    ]
                  ) {
                    let channel = dispositivoRecibido["channel"].filter(
                      (channel) =>
                        channel.channelId ==
                        1 +
                          medicionesFaseActual[i]["grupos"][k]["participantes"][
                            l
                          ]["canal"]
                    )[0];
          ...
          <div>
            <Stepper activeStep={faseActiva} alternativeLabel>
              {fasesExp.map((fase) => (
                <Step key={fase._id}>
                  <StepButton
                    onClick={() => setFaseActiva(fase.numeroFase - 1)}
                  >
                  ...
                  >
                    <Grid item xs={4}>
                      <div className="card">
                        ...
                        <div className="card-body">
                          <h5>
                            ¿Desea continuar a la siguiente Fase/Etapa del
                            Experimento?
                          </h5>
                        </div>
                        <div className="card-footer">
                          <div style={{ float: "right" }}>
                            <Button
                              variant="contained"
                              onClick={() =>
                                closeModalContinuarFase("NoContinuar")
                              }
                              size="small"
                              color="secondary"
                              style={{ margin: 3, textAlign: "center" }}
                            ...
                            <Button
                              ...
                              type="submit"
                              onClick={() =>
                                closeModalContinuarFase("Continuar")
                              }
                              size="small"
                              color="primary"
                              style={{ margin: 3, textAlign: "center" }}
